...
export default function NavBar() {
    ...
    const getLinkClassName = (path: string) => {
        ...
        const activeStyles = theme === 'light' ? styles.active_light : styles.active_dark;
    ...
    const getLink = ({ href, name, className }: { href: string, name: string, className?: string }) => {
        return (
            <Link href={`${href}`} className={getLinkClassName(`${className}`)} onClick={mobileClose}>{name}</Link>
        )
    }
